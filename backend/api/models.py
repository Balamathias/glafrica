...
class Livestock(TimeStampedModel):
    ...
    vaccination_history = models.JSONField(
        default=list, blank=True, help_text="List of vaccines received"
    )
...
class MediaAsset(TimeStampedModel):
    ...
    aspect_ratio = models.FloatField(
        default=1.0, help_text="Width / Height ratio for masonry layout"
    )
...
class Egg(TimeStampedModel):
    ...
    category = models.ForeignKey(EggCategory, on_delete=models.PROTECT, related_name="eggs")
    breed = models.CharField(
        max_length=100, blank=True, help_text="e.g., 'Rhode Island Red', 'Broiler'"
    )
    ...
    price = models.DecimalField(
        max_digits=12, decimal_places=2, null=True, blank=True, help_text="Price per package"
    )
    currency = models.CharField(max_length=3, default="NGN")

    # Inventory
    quantity_available = models.PositiveIntegerField(
        default=0, help_text="Number of packages in stock"
    )

    # Freshness
    production_date = models.DateField(
        null=True, blank=True, help_text="Date eggs were produced/laid"
    )
    ...
    created_by = models.ForeignKey(
        "auth.User", on_delete=models.SET_NULL, null=True, blank=True, related_name="created_eggs"
    )
...
class PageView(TimeStampedModel):
    ...
    session_id = models.CharField(
        max_length=64, db_index=True, help_text="Browser session identifier"
    )
    ...
    livestock = models.ForeignKey(
        "Livestock", on_delete=models.SET_NULL, null=True, blank=True, related_name="page_views"
    )

    # Egg-specific tracking
    egg = models.ForeignKey(
        "Egg", on_delete=models.SET_NULL, null=True, blank=True, related_name="page_views"
    )
