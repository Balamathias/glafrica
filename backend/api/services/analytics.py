...
class AnalyticsService:
    ...
    def get_dashboard_summary():
        ...
        total_revenue = Livestock.objects.filter(is_sold=True, sold_price__isnull=False).aggregate(
            total=Sum("sold_price")
        )["total"] or Decimal("0")

        pending_value = Livestock.objects.filter(is_sold=False).aggregate(total=Sum("price"))[
            "total"
        ] or Decimal("0")

        # This week metrics
        new_this_week = Livestock.objects.filter(created_at__gte=week_ago).count()

        sold_this_week = Livestock.objects.filter(sold_at__gte=week_ago).count()

        revenue_this_week = Livestock.objects.filter(
            sold_at__gte=week_ago, sold_price__isnull=False
        ...
        revenue_last_week = Livestock.objects.filter(
            sold_at__gte=two_weeks_ago, sold_at__lt=week_ago, sold_price__isnull=False
        ).aggregate(total=Sum("sold_price"))["total"] or Decimal("0")
        ...
        conversion_rate = (
            round((sold_livestock / total_livestock * 100), 1) if total_livestock > 0 else 0
        )
    ...
    def get_sales_trend(days=30):
        ...
        sales_by_day = (
            Livestock.objects.filter(sold_at__gte=start_date, is_sold=True)
    ...
    def get_revenue_trend(days=30):
        ...
        while current_date <= end_date:
            result.append(
                {"date": current_date.isoformat(), "revenue": revenue_dict.get(current_date, 0)}
            )
    ...
    def get_inventory_metrics():
        ...
        age_distribution = {
            "young": available.filter(Q(age__icontains="month") | Q(age__icontains="week")).count(),
            "adult": available.filter(Q(age__icontains="year") | Q(age__icontains="yr")).count(),
        }

        # Gender distribution
        gender_distribution = list(available.values("gender").annotate(count=Count("id")))
        ...
        top_categories = list(
            ...
            .order_by("-revenue")[:5]
            .values("name", "sold_count", "revenue")
        )
    ...
    def get_sales_analytics(start_date=None, end_date=None):
        ...
        sold_items = Livestock.objects.filter(
            is_sold=True, sold_at__gte=start_date, sold_at__lte=end_date
        )
        ...
        sales_by_category = list(
            sold_items.values("category__name")
            .annotate(count=Count("id"), revenue=Sum("sold_price"))
            .order_by("-revenue")
        )
        ...
        # Daily breakdown
        daily_sales = list(
            sold_items.annotate(date=TruncDate("sold_at"))
            .values("date")
            .annotate(count=Count("id"), revenue=Sum("sold_price"))
            .order_by("date")
        )
        ...
        # Weekly breakdown
        weekly_sales = list(
            sold_items.annotate(week=TruncWeek("sold_at"))
            .values("week")
            .annotate(count=Count("id"), revenue=Sum("sold_price"))
            .order_by("week")
        )
    ...
    def get_visitor_summary(days=30):
        ...
        bounce_rate = (
            round((bounce_sessions / unique_visitors * 100), 1) if unique_visitors > 0 else 0
        )
        ...
        if sessions_with_duration.exists():
            total_duration = sum(
                (s.last_activity - s.first_visit).total_seconds() for s in sessions_with_duration
            )
        ...
        prev_views = PageView.objects.filter(created_at__gte=prev_start, created_at__lt=start_date)
        prev_sessions = VisitorSession.objects.filter(
            first_visit__gte=prev_start, first_visit__lt=start_date
        )
    ...
    def get_visit_trend(days=30):
        ...
        views_by_day = (
            ...
            .values("date")
            .annotate(visits=Count("id"), unique_sessions=Count("session_id", distinct=True))
            .order_by("date")
        )
        ...
        end_date = timezone.now().date()
        views_dict = {
            item["date"]: {"visits": item["visits"], "unique_visitors": item["unique_sessions"]}
    ...
    def get_top_pages(limit=10, days=30):
        ...
        top_pages = (
            PageView.objects.filter(created_at__gte=start_date, event_type="page_view")
            .values("path")
    ...
    def get_top_livestock_views(limit=10, days=30):
        ...
        top_livestock = (
            PageView.objects.filter(
                created_at__gte=start_date, event_type="livestock_view", livestock__isnull=False
            )
            .values(
                "livestock__id",
                "livestock__name",
            ...
            .annotate(views=Count("id"), unique_viewers=Count("session_id", distinct=True))
            .order_by("-views")[:limit]
        )
    ...
    def get_device_breakdown(days=30):
        ...
        devices = (
            VisitorSession.objects.filter(first_visit__gte=start_date)
            .values("device_type")
    ...
    def get_traffic_sources(limit=10, days=30):
        ...
        # Get referrers
        referrers = (
            PageView.objects.filter(created_at__gte=start_date, referrer__isnull=False)
            .exclude(referrer="")
        ...
        direct_count = (
            PageView.objects.filter(created_at__gte=start_date)
            .filter(Q(referrer__isnull=True) | Q(referrer=""))
            .count()
        )
    ...
    def get_top_egg_views(limit=10, days=30):
        ...
        top_eggs = (
            PageView.objects.filter(
                created_at__gte=start_date, event_type="egg_view", egg__isnull=False
            )
            .values(
                "egg__id",
                "egg__name",
    ...
    def get_egg_dashboard_summary():
        ...
        total_value = Egg.objects.filter(is_available=True).aggregate(
            total=Sum(F("price") * F("quantity_available"))
        )["total"] or Decimal("0")

        # By category
        eggs_by_category = list(
            EggCategory.objects.filter(is_active=True)
            .annotate(
                total=Count("eggs"),
        ...
        # New this week
        new_this_week = Egg.objects.filter(created_at__gte=week_ago).count()

        # Price statistics
        available_queryset = Egg.objects.filter(is_available=True)
        min_price = (
            available_queryset.order_by("price").values_list("price", flat=True).first() or 0
        )
        max_price = (
            available_queryset.order_by("-price").values_list("price", flat=True).first() or 0
        )
    ...
    def get_eggs_by_category():
        ...
        categories = (
            ...
            .values("id", "name", "slug", "total", "available", "total_units", "value")
            .order_by("order")
        )
    ...
    def get_expiring_eggs(days=7):
        ...
        expiring = (
            Egg.objects.filter(
                is_available=True, expiry_date__lte=threshold_date, expiry_date__gte=now.date()
            )
